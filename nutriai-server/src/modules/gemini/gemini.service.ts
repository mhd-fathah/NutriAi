...
export class GeminiService {
  ...
  private cleanAndParseJSON(text: string): Omit<NutritionAnalysis, 'isEstimated' | 'aiStatus' | 'aiProvider'> {
  ...
  private async callGeminiModel(
    ...
    imageBase64: string,
    mimeType: string,
  ): Promise<Omit<NutritionAnalysis, 'isEstimated' | 'aiStatus' | 'aiProvider'>> {
  ...
  async analyzeFood(
    ...
    // Phase 1: Try Primary model (gemini-2.5-flash) with 3 retries
    for (let attempt = 0; attempt <= primaryRetries; attempt++) {
      try {
        ...
        this.logger.error(`Primary model attempt ${attempt} failed: ${err.message}`);
      }
    }

    // Phase 2: Fallback model (gemini-2.0-flash)
    try {
      ...
      this.logger.error(`Fallback Model ${this.fallbackModel} failed: ${err.message}`);
    }

    // Phase 3: Fallback estimated nutrition
